...
function hasEquivalentParagraphWithFigure(dynamicContent, introBlocks, figure) {
  if (!Array.isArray(dynamicContent)) return false;

  const expectedIntro = normalizeBlocks(introBlocks);
  ...
  return dynamicContent.some((item) => {
    ...
    const existingIntro = normalizeBlocks(item.content?.content);
    const existingFigure = normalizeFigure(item.figure);

    return (
      stableStringify(existingIntro) === stableStringify(expectedIntro) &&
      stableStringify(existingFigure) === stableStringify(expectedFigure)
    );
  });
}

function buildParagraphWithFigure(introBlocks, figure) {
  const next = {
    __component: TARGET_COMPONENT,
    imageSide: "right",
    content: {
      content: normalizeBlocks(introBlocks),
    },
  };
...
async function up() {
  ...
  for (const slot of slots) {
    const introBlocks = normalizeBlocks(slot.introContent);
    const firstFigure =
      Array.isArray(slot.images) && slot.images.length > 0
        ? slot.images[0]
        : null;

    const hasIntro = introBlocks.length > 0;
    const hasFigure = normalizeFigure(firstFigure) !== null;

    if (!hasIntro && !hasFigure) {
      skipped += 1;
      continue;
    }
    ...
    const nextComponent = buildParagraphWithFigure(introBlocks, firstFigure);
  ...
  strapiInstance.log.info(
    `[slot migration] Appended ${updated} paragraph-with-figure blocks from introContent/images; skipped ${skipped}.`,
  );
}
